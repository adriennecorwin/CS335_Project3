...
public class MorphTools
{
            ...
            Object INTERPOLATION)
    {
        ...
        for (int i = 0; i < 3; ++i) {
            a[i][0] = S.getV(i+1).getX();

            // System.out.println("P" + i + "(" + S.getX(i) + ", " + S.getY(i) +
            // ")" );

            a[i][1] = S.getV(i+1).getY();
            a[i][2] = 1.0;
        }
        ...
        destPath.moveTo((float)D.getV(1).getX(), (float)D.getV(1).getY());
        destPath.lineTo((float)D.getV(2).getX(), (float)D.getV(2).getY());
        destPath.lineTo((float)D.getV(3).getX(), (float)D.getV(3).getY());
        destPath.lineTo((float)D.getV(1).getX(), (float)D.getV(1).getY());
